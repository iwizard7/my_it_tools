...
<main>
  ...
  <footer>ESP32-C3 · local-only tools · Wi-Fi AP: ESP32-Random-Tools · v1.8.2</footer>
</main>
...
const helpText={random:['Random data','Генерирует строки аппаратным RNG ESP32. Длина 1–256, до 20 результатов.'],uuid:['UUID generator','Создаёт UUID v4. Используйте для request ID, correlation ID и тестовых сущностей.'],token:['Token & password','Генерирует тестовые токены и пароли. Не используйте их как production secrets без дополнительного хранения.'],base64:['Base64','Кодирует и декодирует UTF-8. Base64 не является шифрованием.'],url:['URL encoder','Кодирует query-параметры и декодирует URL-строки.'],json:['JSON tools','Format проверяет JSON и делает его читаемым, Minify убирает лишние пробелы.'],stats:['Text statistics','Считает строки, слова, символы и UTF-8 bytes.'],toolbox:['DevOps Toolkit','Локальные helpers для curl, Git, Docker, Kubernetes, Linux, CI/CD, SRE и security.'],qr:['QR generator','Создаёт QR на ESP32. Подходит для URL, Wi-Fi payload и коротких текстов.'],network:['Network diagnostics','Быстрые DNS, route/DHCP, TCP, HTTP, mDNS, Wi-Fi и IPv4 проверки. Для сложного DNS используйте DNS Inspector на Debian Gateway.'],api:['API console','Мини-Postman для HTTP через ESP32. HTTPS/TLS-проверка выполняется в Debian Gateway.'],logs:['Log analyzer','Загрузите локальный лог до 2 MB или вставьте текст. Файл не покидает браузер.'],incident:['Incident probe','Последовательно проверяет gateway, DNS, TCP и HTTP и формирует JSON incident report.'],system:['Metrics & device','Показывает RAM, uptime, температуру, LittleFS, Wi-Fi и OTA.']};
